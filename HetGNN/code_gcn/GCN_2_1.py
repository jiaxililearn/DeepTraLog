...
class HetGCN_2_1(nn.Module):
    def __init__(self, model_path=None, feature_size=7, out_embed_s=32, num_node_types=7, hidden_channels=16, **kwargs):
        ...
        """
        super(HetGCN_2_1, self).__init__()
        ...
        self.conv1 = GCNConv(self.embed_d, 64)
        self.conv2 = GCNConv(64, self.out_embed_d)
    ...
    def forward(self, data):
        ...
        """
        x_node_feature, x_edge_index, x_edge_weight, _ = data

        # print(f'x_node_feature shape: {x_node_feature.shape}')
        # print(f'x_edge_index shape: {x_edge_index.shape}')
        h = self.conv1(x_node_feature, x_edge_index, edge_weight=x_edge_weight)
        h = self.relu(h)
        h = self.conv2(h, x_edge_index, edge_weight=x_edge_weight)
        h = h.sigmoid()
