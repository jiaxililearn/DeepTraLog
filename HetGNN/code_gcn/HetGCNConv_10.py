...
class HetGCNConv_10(MessagePassing):
    """
    self implemented 
    """
    def __init__(self, in_channels, out_channels, num_node_types, hidden_channels=16,
                 num_hidden_conv_layers=1, num_src_types=2, num_edge_types=1):
        super().__init__()
        ...
        self.fc_het_layer = torch.nn.Linear(hidden_channels * num_node_types * num_src_types * num_edge_types, out_channels, bias=True)

        self.node_fc_het_layer = torch.nn.Linear(hidden_channels * num_node_types * num_src_types * num_edge_types, out_channels, bias=True)
    ...
    def forward(self, graph_data, source_types=[0, 1]):
        ...
        for ntype in range(self.num_node_types * self.num_src_types):
            for etype in range(self.num_edge_types):
                ...
                _, het_edge_index, het_edge_weight = self.get_het_edge_index(
                    edge_index, edge_weight, node_types, ntype,
                    source_types=source_types, edge_type_list=edge_type, edge_type=etype)
        ...
        combined_het_embedding = torch.cat(het_h_embeddings, 1).view(node_feature.shape[0], self.hidden_channels * self.num_node_types * self.num_src_types * self.num_edge_types)

        # getting individual node embedding
        h_node = self.relu(self.node_fc_het_layer(combined_het_embedding))
    ...
    def get_het_edge_index(self, edge_index, edge_weight, node_types, ntype, source_types=None,
                           edge_type_list=None, edge_type=None):
        """
