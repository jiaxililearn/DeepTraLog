    ...
    def __getitem__(self, gid):
        ...
        if self.include_edge_type:
            edge_type = torch.from_numpy(
                self.edge_inedx_df[self.edge_inedx_df.trace_id == gid]['edge_type'].values.reshape(-1,)
            ).float().to(self.device)
            return graph_node_feature, edge_index, (edge_weight, edge_type), self.node_types[gid]
