    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.model_version == 13:
            from GCN_13 import HetGCN_13 as HetGCN
        ...
        self.random_target.init_weights()
        ...
        for iter_i in range(self.train_iter_n):
            ...
            for batch_n, k in tqdm(enumerate(batch_list)):
                ...
                if self.main_loss == "semi-svdd":
                    ...
                    _out_h_random_target = torch.zeros(
                        int(self.batch_s / self.mini_batch_s),
                        self.mini_batch_s,
                        self.out_embed_d,
                    ).to(self.device)
                ...
                _out_h_node = torch.tensor(_out_h_node).to(self.device)

                # detach to skip backward
                _out_h_random_target = _out_h_random_target.detach()
                _out_h_node_random_target = torch.tensor(_out_h_node_random_target).to(self.device).detach()
